...
struct Array[dtype:DType,opt_nelts:Int]:
    ...
    fn __copyinit__(inout self, other: Self):
        ...
        # let nc: Int = num_cores()
        # let chunk_size = opt_nelts*(self.size//(opt_nelts*nc))

        # @parameter
        # fn simd_arith(core:Int):
        #     # let ind:Int = core * opt_nelts//num_cores()
        #     for i in range((core)*chunk_size, chunk_size*(core+1), opt_nelts):
        #         let other_data = other.data.simd_load[opt_nelts](i)
        #         self.data.simd_store[opt_nelts](i,other_data)
        
        # # let rt =Runtime()
        # parallelize[simd_arith]()
        # # rt._destroy()
        # let par_rem = self.size%(chunk_size*nc)
        # if par_rem != 0 :
        #     for i in range(chunk_size*nc,self.size,opt_nelts):
        #         let other_data = other.data.simd_load[opt_nelts](i)
        #         self.data.simd_store[opt_nelts](i,other_data)
        #     if par_rem%opt_nelts !=0:
        #         for i in range(chunk_size*nc+opt_nelts*par_rem, self.size):
        #             let other_data = other.data.simd_load[1](i)
        #             self.data.simd_store[1](i,other_data)
...
#                 self[i] = val
        ...
        if par_rem != 0 :
            ...
            if par_rem%opt_nelts !=0:
                for i in range(chunk_size*nc+opt_nelts*par_rem, self.size):
                    ...
                    self.data.simd_store(i,func[dtype,1](simd_data_self,simd_data_rhs))
        ...
        if par_rem != 0 :
            ...
            if par_rem%opt_nelts !=0:
                for i in range(chunk_size*nc+opt_nelts*par_rem, self.size):
                    ...
                    self.data.simd_store(i,func[dtype,1](simd_data_self,rhs))
    ...
    fn arithmatic_Array_SIMD[func: fn[type:DType, simd_w:Int](SIMD[type, simd_w],SIMD[type, simd_w])->SIMD[type, simd_w]](self, rhs: SIMD[dtype,1])raises->Array[dtype,opt_nelts]:
        ...
        fn simd_arith(core:Int):
            for i in range((core)*chunk_size, chunk_size*(core+1), opt_nelts):
                let simd_data_self = self.data.simd_load[opt_nelts](i)
                result_array.data.simd_store[opt_nelts](i, func[dtype,opt_nelts](simd_data_self, rhs))
        parallelize[simd_arith]()
        ...
        if par_rem != 0 :
            ...
            if par_rem%opt_nelts !=0:
                for i in range(chunk_size*nc+opt_nelts*par_rem, self.size):
                    ...
                    result_array.data.simd_store(i,func[dtype,1](simd_data_self,rhs))
        return result_array
    ...
    fn arithmatic_Array_Array[func: fn[type:DType, simd_w:Int](SIMD[type, simd_w],SIMD[type, simd_w])->SIMD[type, simd_w]](self, rhs: Array[dtype,opt_nelts])raises->Array[dtype,opt_nelts]:
        ...
        fn simd_arith(core:Int):
            for i in range((core)*chunk_size, chunk_size*(core+1), opt_nelts):
                let simd_data_self = self.data.simd_load[opt_nelts](i)
                let simd_data_rhs = rhs.data.simd_load[opt_nelts](i)
                result_array.data.simd_store[opt_nelts](i, func[dtype,opt_nelts](simd_data_self, simd_data_rhs))
        ...
        if par_rem != 0 :
            ...
            if par_rem%opt_nelts !=0:
                for i in range(chunk_size*nc+opt_nelts*par_rem, self.size):
                    let simd_data_self = self.data.load(i)
                    let simd_data_rhs = rhs.data.load(i)
                    result_array.data.simd_store(i,func[dtype,1](simd_data_self,simd_data_rhs))
        return result_array
    ...
    fn math_func[func: fn[type:DType, simd_w:Int](SIMD[type, simd_w]) -> SIMD[type, simd_w]](self,arr:Array[dtype,opt_nelts])->Array[dtype,opt_nelts]:
        let res_arr:Array[dtype,opt_nelts]=Array[dtype,opt_nelts](self.rows,self.cols)
        let nc: Int = num_cores()
        let chunk_size = opt_nelts*(self.size//(opt_nelts*nc))
        ...
        if par_rem != 0 :
            ...
            if par_rem%opt_nelts !=0:
                for i in range(chunk_size*nc+opt_nelts*par_rem, self.size):
                    let simd_data = res_arr.data.simd_load[1](i)
                    res_arr.data.simd_store[1](i, func[dtype,1](simd_data))
        return res_arr
