...
struct Array[dtype:DType,opt_nelts:Int]:
    ...
    var tshape: TensorShape
    var size: Int
    var dim_list: VariadicList[Int]
    ...
    fn __init__(inout self):
        self.tshape = TensorShape(1)
        self.data = Tensor[dtype](self.tshape)
        self.dim_list = VariadicList[Int](1)
        self.size = self.tshape.num_elements()        
        self.zero()
        
        
    fn __copyinit__(inout self, other: Self):
        self.tshape = other.tshape
        self.data = other.data
        self.dim_list = other.dim_list
        self.size = self.tshape.num_elements()
        
        for i in range(0, opt_nelts*(self.size//opt_nelts), opt_nelts):
            var other_data = other.data.load[opt_nelts](i)
            self.data.store[opt_nelts](i,other_data)
        if self.size%opt_nelts != 0 :    
            for i in range(opt_nelts*(self.size//opt_nelts), self.size):
                var other_data = other.data.load[1](i)
    ...
    fn zero(inout self):
        ...
        for i in range(0, opt_nelts*(self.size//opt_nelts), opt_nelts):
            self.data.store[opt_nelts](i, zero) 
    ...
    @always_inline
    fn dim_check_int(self,index:VariadicList[Int])raises:
        for i in range(self.dim_list.__len__()):
            if index[i]>(self.dim_list[i]-1):
    ...
    #     var new_Arr: Array[dtype,opt_nelts] = Array[dtype,opt_nelts](new_cols,1)
    ...
    #     var new_Arr: Array[dtype,opt_nelts] = Array[dtype,opt_nelts](new_rows,1)
    ...
    #     var new_cols:Int = xspan.__len__()
    #     var new_rows:Int = yspan.__len__()
    #     var new_Arr: Array[dtype,opt_nelts] = Array[dtype,opt_nelts](new_rows,new_cols)
    #     for i in range(new_cols):
    ...
    #     var new_Arr: Array[dtype,opt_nelts] = Array[dtype,opt_nelts](new_size,1)
...
#                 self[i] = val
    ...
    @always_inline
    fn __setitem__(inout self, index:VariadicList[Int], val: SIMD[dtype,1]) raises:
        self.dim_check_int(index)
        self.data[index]=val
    ...
    #     return self.data.store[1]( x, val)
    ...
    #     var new_Arr: Array[dtype,opt_nelts] = self
    #     for i in range(new_size): 
    ...
    #     var new_size:Int = span.__len__()
    #     for i in range(new_size): 
    #         self.data.store[1](i,val)
    
    #Additional Methods
    
    # @always_inline
    # fn transpose(self) raises ->Array[dtype,opt_nelts]:
    #     var result_array: Array[dtype,opt_nelts] = Array[dtype,opt_nelts](self.dim_list)
    #     if self.dim_list.__len__() >2:
    #         raise Error("Transpose not implemented for dims higher than 2")
    #     for i in range(self.dim_list[0]):
    #         for j in range(self.dim_list[1]):
    #             result_array[j,i]=self[i,j]
    #     return result_array
    
    # @always_inline
    # fn get_shape(self) raises:
    #     print("shape ", self.dim_list)
    
    # @always_inline
    # fn load[nelts:Int](self, y: Int, x: Int) raises -> SIMD[dtype, nelts]:

    #     return self.data.load[nelts](y * self.cols + x)
    ...
    #     self.data.store[nelts](y * self.cols + x, val)
    ...
    fn iarithmatic_Array_Array[func: fn[type:DType, simd_w:Int](SIMD[type, simd_w],SIMD[type, simd_w])->SIMD[type, simd_w]](inout self, rhs: Array[dtype,opt_nelts])raises:
        # @parameter
        # fn simd_arith(i:Int):
        #     var ind:Int = i * opt_nelts
        for i in range(0, opt_nelts*(self.size//opt_nelts), opt_nelts):
            var simd_data_self = self.data.load[opt_nelts](i)
            var simd_data_rhs = rhs.data.load[opt_nelts](i)
            self.data.store[opt_nelts](i, func[dtype,opt_nelts](simd_data_self, simd_data_rhs))
        ...
        if self.size%opt_nelts != 0 :    
            for i in range(opt_nelts*(self.size//opt_nelts), self.size):
                var simd_data_self = self.data.load[1](i)
                var simd_data_rhs = rhs.data.load[1](i)
    ...
    fn iarithmatic_Array_SIMD[func: fn[type:DType, simd_w:Int](SIMD[type, simd_w],SIMD[type, simd_w])->SIMD[type, simd_w]](inout self, rhs:SIMD[dtype,1])raises:
        ...
        for i in range(0, opt_nelts*(self.size//opt_nelts), opt_nelts):
            var simd_data_self = self.data.load[opt_nelts](i)
            self.data.store[opt_nelts](i, func[dtype,opt_nelts](simd_data_self, rhs))
        ...
        if self.size%opt_nelts != 0 :    
            for i in range(opt_nelts*(self.size//opt_nelts), self.size):
                ...
                self.data.store[1](i, func[dtype,1](simd_data_self, rhs))
    ...
    fn arithmatic_Array_SIMD[func: fn[type:DType, simd_w:Int](SIMD[type, simd_w],SIMD[type, simd_w])->SIMD[type, simd_w]](self, rhs: SIMD[dtype,1])raises->Array[dtype,opt_nelts]:
        ...
        #     var ind:Int = i * opt_nelts
        for i in range(0, opt_nelts*(self.size//opt_nelts), opt_nelts):
            var simd_data = self.data.load[opt_nelts](i)
            result_array.data.store[opt_nelts](i, func[dtype,opt_nelts](simd_data, rhs))
        ...
        if self.size%opt_nelts != 0 :    
            for i in range(opt_nelts*(self.size//opt_nelts), self.size):
                var simd_data = self.data.load[1](i)
    ...
    fn arithmatic_Array_Array[func: fn[type:DType, simd_w:Int](SIMD[type, simd_w],SIMD[type, simd_w])->SIMD[type, simd_w]](self, rhs: Array[dtype,opt_nelts])raises->Array[dtype,opt_nelts]:
        ...
        for i in range(0, opt_nelts*(self.size//opt_nelts), opt_nelts):
            var simd_data_self = self.data.load[opt_nelts](i)
            var simd_data_rhs = rhs.data.load[opt_nelts](i)
            result_array.data.store[opt_nelts](i, func[dtype,opt_nelts](simd_data_self, simd_data_rhs))
    ...
    fn math_func[func: fn[type:DType, simd_w:Int](SIMD[type, simd_w]) -> SIMD[type, simd_w]](self,arr:Array[dtype,opt_nelts])->Array[dtype,opt_nelts]:
        var res_arr:Array[dtype,opt_nelts]=arr
        # var arr_c = arr
        # @parameter
        ...
        # fn simd_arith(i:Int):
            ...
            var simd_data = res_arr.data.load[opt_nelts](i)
            res_arr.data.store[opt_nelts](i,func[dtype,opt_nelts](simd_data))
        ...
        if arr.size%opt_nelts != 0:
            for i in range(opt_nelts*(arr.size//opt_nelts), arr.size):
                var simd_data = func[dtype,1]( arr.data.load[1](i))
                res_arr.data.store[1](i, simd_data)
        return res_arr
    ...
    fn __ipow__(inout self, rhs: Int)raises:
        for i in range(0, opt_nelts*(self.size//opt_nelts), opt_nelts):
            var simd_data = self.data.load[opt_nelts](i)
            self.data.store[opt_nelts](i, simd_data ** rhs)   
        if self.size%opt_nelts != 0 :    
            for i in range(opt_nelts*(self.size//opt_nelts), self.size):
                var simd_data = self.data.load[1](i)
                self.data.store[1](i, simd_data ** rhs)
    ...
    fn __pow__(self, rhs: Int)raises->Array[dtype,opt_nelts]:
        ...
        for i in range(0, opt_nelts*(self.size//opt_nelts), opt_nelts):
            var simd_data = self.data.load[opt_nelts](i)
            result_array.data.store[opt_nelts](i, simd_data ** rhs)
        if self.size%opt_nelts != 0 :    
            for i in range(opt_nelts*(self.size//opt_nelts), self.size):
                var simd_data = self.data.load[1](i)
    ...
    fn sum(self, arr:Array[dtype,opt_nelts])raises->SIMD[dtype,1]:
        ...
        for i in range(0, opt_nelts*(arr.size//opt_nelts), opt_nelts):
            var simd_data = arr.data.load[opt_nelts](i)
    ...
    #         var number_of_steps: SIMD[dtype,1] = diff/step
    #         var int_number_of_steps: Int = number_of_steps.cast[DType.int32]().to_int() + 1
    #         var arr: Array[dtype,opt_nelts]=Array[dtype,opt_nelts](int_number_of_steps,1)
    #     # arr.fill(start)
    ...
    #     var ident: Array[dtype, opt_nelts] = Array[dtype, opt_nelts](n,n)
    #     for i in range(n):
    #         var ind: VariadicList[Int] =VariadicList[Int](i,i)
    #         ident[ind] = 1
    #     return ident
    
    fn zeros(self,*index:Int)raises->Array[dtype,opt_nelts]:
        return Array[dtype,opt_nelts](index)

    fn zeros(self,index:VariadicList[Int])raises->Array[dtype,opt_nelts]:
        return Array[dtype,opt_nelts](index) 
    
    fn ones(self,*index:Int)raises->Array[dtype,opt_nelts]:
        var arr: Array[dtype,opt_nelts] = Array[dtype,opt_nelts](index)
        arr.fill(1)
        return arr
    
    fn ones(self,index:VariadicList[Int])raises->Array[dtype,opt_nelts]:
        var arr: Array[dtype,opt_nelts] = Array[dtype,opt_nelts](index)
        arr.fill(1)
        return arr
    ...
    fn arr_print(self)raises:
        ...
        #         if j != (self.dim_list[0] - 1):
        #             print_no_newline(", ")
        #     print_no_newline("]")
        #     put_new_line()
    # def from_tensor(self,tens:Tensor[dtype])->Array[dtype,opt_nelts]:
    #     var arr:Array[dtype,opt_nelts]
    #     arr.data = tens
    #     arr.tshape = tens.shape()
    #     return arr
        # var t=Tuple[Int](1)
        # for i in range(arr.tshape.rank()):
        #     t[i] = arr.tshape[i]
    # def from_numpy(self,input:PythonObject)->Array[dtype,opt_nelts]:
    #     np = Python.import_module("numpy")
    #     np_array=np.array(input)
    #     var np_pointer = DTypePointer[DType.float64](
    ...
    #         )
    #     var arr: Self
    #     arr.data =Tensor[dtype](np_pointer)
    #     arr.size = Int(np_array.size.)
    #     arr.dim_list = VariadicList[Int](np_array.shape)
    #     return arr
